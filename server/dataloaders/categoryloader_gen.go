...
type CategoryLoaderConfig struct {
	...
	Fetch func(keys []string) ([]*db.Category, []error)
...
func (l *CategoryLoader) Load(key string) (*db.Category, error) {
...
func (l *CategoryLoader) LoadThunk(key string) func() (*db.Category, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*db.Category, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *CategoryLoader) LoadAll(keys []string) ([]*db.Category, []error) {
	results := make([]func() (*db.Category, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	categorys := make([]*db.Category, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *CategoryLoader) LoadAllThunk(keys []string) func() ([]*db.Category, []error) {
	results := make([]func() (*db.Category, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *CategoryLoader) Prime(key string, value *db.Category) bool {
	l.mu.Lock()
	var found bool
...
func (l *CategoryLoader) unsafeSet(key string, value *db.Category) {
	if l.cache == nil {
		l.cache = map[string]*db.Category{}
	}
	l.cache[key] = value
}
