...
type ProductLoaderConfig struct {
	...
	Fetch func(keys []string) ([]*db.Product, []error)
...
func (l *ProductLoader) Load(key string) (*db.Product, error) {
...
func (l *ProductLoader) LoadThunk(key string) func() (*db.Product, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*db.Product, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *ProductLoader) LoadAll(keys []string) ([]*db.Product, []error) {
	results := make([]func() (*db.Product, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	products := make([]*db.Product, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ProductLoader) LoadAllThunk(keys []string) func() ([]*db.Product, []error) {
	results := make([]func() (*db.Product, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ProductLoader) Prime(key string, value *db.Product) bool {
	l.mu.Lock()
	var found bool
...
func (l *ProductLoader) unsafeSet(key string, value *db.Product) {
	if l.cache == nil {
		l.cache = map[string]*db.Product{}
	}
	l.cache[key] = value
}
