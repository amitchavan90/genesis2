...
func canonicalLogger(logger *zap.Logger) func(http.Handler) http.Handler {
	return func(next http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			now := time.Now()
			ctx := genesis.WithCanonicalLogger(r.Context(), logger)
			...
			canlog.Set(ctx, "req_body", ioutil.NopCloser(bytes.NewBuffer(bodyBytes)))

			next.ServeHTTP(w, r.WithContext(ctx))
			canlog.Set(ctx, "duration_ms", fmt.Sprintf("%.02f", float64(time.Since(now).Microseconds())/1000))

			if genesis.ClaimExistsInContext(ctx, genesis.ClaimUserName) {
				username, _ := genesis.ClaimValueFromContext(ctx, genesis.ClaimUserName)
				canlog.Set(ctx, "username", username)
			}
			if genesis.ClaimExistsInContext(ctx, genesis.ClaimRoleID) {
				roles, _ := genesis.ClaimValueFromContext(ctx, genesis.ClaimRoleID)
				canlog.Set(ctx, "role_id", roles)
			}
			if genesis.ClaimExistsInContext(ctx, genesis.ClaimUserID) {
				userID, _ := genesis.ClaimValueFromContext(ctx, genesis.ClaimUserID)
...
func debugClientInput() func(http.Handler) http.Handler {
	return func(next http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			...
			log.Println("## debug:http", bodyString)
