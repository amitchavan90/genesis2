...
func CheckValidEmail(str string) error {
	if len(str) == 0 {
		return terror.New(fmt.Errorf("IsValidEmail len: %w", errors.New("email cannot be blank")), "")
	}

	if !regexEmailAddr.MatchString(str) {
		return terror.New(fmt.Errorf("IsValidEmail match: %w", errors.New("invalid email address string")), "")
	}
...
func CheckValidProcoreAPI(str string) error {
	if !regexProcoreAPIFormat.MatchString(str) {
		return terror.New(fmt.Errorf("Invalid Procore api matched: %w", errors.New("invalid procore api format")), "")
	}
	return nil
}
