...
type SKUStorer interface {
	...
	InsertRetailLink(retailLink *db.RetailLink, txes ...*sql.Tx) (*db.RetailLink, error)
...
type TaskStorer interface {
	...
	Insert(record *db.Task, tx ...*sql.Tx) (*db.Task, error)
	InsertSubtask(st *db.Subtask, txes ...*sql.Tx) (*db.Subtask, error)
