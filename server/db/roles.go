...
type Role struct {
	...
	Permissions types.StringArray `db:"permissions" boil:"permissions" json:"permissions" toml:"permissions" yaml:"permissions"`
	Tier        int               `db:"tier" boil:"tier" json:"tier" toml:"tier" yaml:"tier"`
...
var RoleWhere = struct {
	...
	Archived:    whereHelperbool{field: "\"roles\".\"archived\""},
	ArchivedAt:  whereHelpernull_Time{field: "\"roles\".\"archived_at\""},
...
var (
	roleAllColumns            = []string{"id", "name", "permissions", "tier", "archived", "archived_at", "updated_at", "created_at"}
	roleColumnsWithoutDefault = []string{"name", "permissions", "archived_at"}
	roleColumnsWithDefault    = []string{"id", "tier", "archived", "updated_at", "created_at"}
...
// TrackActions retrieves all the track_action's TrackActions with an executor.
func (o *Role) TrackActions(mods ...qm.QueryMod) trackActionQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"role_track_actions\" on \"track_actions\".\"id\" = \"role_track_actions\".\"track_action_id\""),
		qm.Where("\"role_track_actions\".\"role_id\"=?", o.ID),
	)
...
// LoadTrackActions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (roleL) LoadTrackActions(e boil.Executor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(
		qm.Select("\"track_actions\".*, \"a\".\"role_id\""),
		qm.From("\"track_actions\""),
		qm.InnerJoin("\"role_track_actions\" as \"a\" on \"track_actions\".\"id\" = \"a\".\"track_action_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.RequirePhotos, &one.Code, &one.Name, &one.NameChinese, &one.Private, &one.System, &one.Blockchain, &one.Archived, &one.ArchivedAt, &one.UpdatedAt, &one.CreatedAt, &one.CreatedByID, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for track_actions")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice track_actions")
		}
	...
	if singular {
		object.R.TrackActions = resultSlice
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.TrackActions = append(local.R.TrackActions, foreign)
				if foreign.R == nil {
					foreign.R = &trackActionR{}
				}
...
// AddTrackActions adds the given related objects to the existing relationships
// of the role, optionally inserting them as new records.
// Appends related to o.R.TrackActions.
// Sets related.R.Roles appropriately.
func (o *Role) AddTrackActions(exec boil.Executor, insert bool, related ...*TrackAction) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"role_track_actions\" (\"role_id\", \"track_action_id\") values ($1, $2)"
	...
	if o.R == nil {
		o.R = &roleR{
			TrackActions: related,
		}
	} else {
		o.R.TrackActions = append(o.R.TrackActions, related...)
	}
...
// SetTrackActions removes all previously related items of the
// role replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Roles's TrackActions accordingly.
// Replaces o.R.TrackActions with related.
// Sets related.R.Roles's TrackActions accordingly.
func (o *Role) SetTrackActions(exec boil.Executor, insert bool, related ...*TrackAction) error {
	query := "delete from \"role_track_actions\" where \"role_id\" = $1"
	...
	removeTrackActionsFromRolesSlice(o, related)
	if o.R != nil {
		o.R.TrackActions = nil
	}
	return o.AddTrackActions(exec, insert, related...)
}

// RemoveTrackActions relationships from objects passed in.
// Removes related items from R.TrackActions (uses pointer comparison, removal does not keep order)
// Sets related.R.Roles.
func (o *Role) RemoveTrackActions(exec boil.Executor, related ...*TrackAction) error {
	var err error
	query := fmt.Sprintf(
		"delete from \"role_track_actions\" where \"role_id\" = $1 and \"track_action_id\" in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.TrackActions {
			if rel != ri {
				continue
			}

			ln := len(o.R.TrackActions)
			if ln > 1 && i < ln-1 {
				o.R.TrackActions[i] = o.R.TrackActions[ln-1]
			}
			o.R.TrackActions = o.R.TrackActions[:ln-1]
			break
		}
	}
...
func removeTrackActionsFromRolesSlice(o *Role, related []*TrackAction) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
